    ...
    // Main.
        ...
        $.getJSON('images/images.json', function(images) {
            ...
            function openImagePopup(filename) {
                let $link = $(`a.image[data-file="${filename}.webp"]`);
                if ($link.length) {
                    $link.trigger('click');
                }
            }
            ...
            function applyFilter(year) {
                ...
                document.dispatchEvent(new CustomEvent('galleryReady', { detail: { year } }));
            }
            ...
            function initFromHash() {
                ...
                if (hash) {
                    if (/^\d{4}$/.test(hash)) {
                        ...
                        const onReady = () => {
                            openImagePopup(hash);
                            document.removeEventListener('galleryReady', onReady);
                        };

                        document.addEventListener('galleryReady', onReady);
